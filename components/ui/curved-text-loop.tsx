...
const CurvedTextLoop: FC<CurvedTextLoopProps> = ({
        ...
        (ctx: CanvasRenderingContext2D, offset: number) => {
            ...
            for (let i = 0; i < repeatCount; i++) {
                ...
                for (let j = 0; j < processedText.length; j++) {
                    ...
                    const t = normalizedX;
                    const curveY =
                        canvasHeight / 2 - curveHeight * Math.sin(t * Math.PI);
    ...
    useEffect(() => {
        if (!isReady || !canvasRef.current || !dimensions.width || !textWidth)
            return;
        ...
        const animate = (currentTime: number) => {
            ...
            offsetRef.current +=
                (targetOffsetRef.current - offsetRef.current) * easing;
...
CurvedTextLoop.displayName = "CurvedTextLoop";
